...
| `rns.link.request` | `destination_hash`, `aspect`, `path`, `data_b64?`, `timeout?`, `request_id` | Generic request/response over an RNS Link — drop-in replacement for `nomadnet.page.download` with configurable `aspect`. Opens the link if not already cached; streams `phase`/`progress` then final `success` (body returned as `body_b64`) or `failure`. |
